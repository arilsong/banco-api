...
public class OutboundTransferService {
     ...
     * Recebe fromAccount (account ID) e toAccount (MSISDN do destinatário).
     * Resolve o MSISDN do remetente internamente antes de chamar o SDK.
     */
    public TransferInitiateResponse initiateTransfer(TransferInitiateRequest request) {
        // Resolve o MSISDN do remetente: aceita account ID ("DO0001") ou MSISDN ("23000000001") diretamente
        ...
        String amount = new BigDecimal(request.getAmount()).stripTrailingZeros().toPlainString();
        body.put("amount", amount);
        body.put("transactionType", "TRANSFER");

        try {
            log.info("Iniciando Step 1 (Lookup) no SDK [CAIXA]: from={} to={}", fromMsisdn, request.getToAccount());
            ...
            if (respBody == null || "ERROR_OCCURRED".equals(respBody.get("currentState"))) {
                throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "SDK retornou erro no Step 1.");
            }
            ...
            log.error("Erro no Step 1 [CAIXA]: {}", e.getMessage());
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage());
        }
    }

    /**
     * Passo 2: Confirmar Destinatário (Accept Party) e receber cotação
     */
    public TransferConfirmPartyResponse confirmParty(String transferId) {
        Map<String, Object> body = Map.of("acceptParty", true);
        try {
            log.info("Iniciando Step 2 (Accept Party) no SDK [CAIXA] para ID: {}", transferId);
            ...
            if (respBody == null || "ERROR_OCCURRED".equals(respBody.get("currentState"))) {
                throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "SDK retornou erro no Step 2.");
            }
            ...
            log.error("Erro no Step 2 [CAIXA]: {}", e.getMessage());
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage());
        }
    }

    /**
     * Passo 3: Confirmar Taxa (Accept Quote) e executar transferência
     */
    public TransferConfirmQuoteResponse confirmQuote(String transferId) {
        Map<String, Object> body = Map.of("acceptQuote", true);
        try {
            log.info("Iniciando Step 3 (Accept Quote) no SDK [CAIXA] para ID: {}", transferId);
            ResponseEntity<Map> response = restTemplate.exchange(
                    sdkOutboundUrl + "/transfers/" + transferId,
                    HttpMethod.PUT,
                    new HttpEntity<>(body),
            ...
            if (respBody == null || "ERROR_OCCURRED".equals(respBody.get("currentState"))) {
                throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "SDK retornou erro no Step 3.");
            }

            String currentState = (String) respBody.get("currentState");

            if ("COMMITTED".equals(currentState) || "COMPLETED".equals(currentState)) {
                log.info("[CAIXA] Step 3 concluído com sucesso. Processando débito local...");
                ...
                transferService.processTransfer(localReq);
            }
            ...
            log.error("Erro no Step 3 [CAIXA]: {}", e.getMessage());
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage());
        }
    }
}
