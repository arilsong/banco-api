...
import com.caixa.api.dto.OutboundTransferRequest;
import com.caixa.api.service.PartyService;
import com.caixa.api.service.SdkOutboundClientService;
...
public class DirectSendController {
    ...
    public ResponseEntity<Object> lookupParty(@PathVariable String type, @PathVariable String id) {
        log.info("Lookup direto [CAIXA]: {}/{}", type, id);
    ...
    public ResponseEntity<Object> requestQuote(@RequestBody Map<String, Object> quoteRequest) {
        log.info("Quote direto [CAIXA]: {}", quoteRequest);
    ...
    public ResponseEntity<String> sendTransfer(@RequestBody OutboundTransferRequest request) {
        log.info("Envio direto [CAIXA]: {} → {}", request.getFromIdValue(), request.getToIdValue());
        ...
        if (success) {
            log.info("Envio direto concluído com sucesso [CAIXA].");
            return ResponseEntity.ok("Transferência enviada com sucesso.");
        } else {
            log.error("Envio direto falhou [CAIXA]. Estornando saldo.");
