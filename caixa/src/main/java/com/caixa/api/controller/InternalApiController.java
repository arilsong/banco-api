...
public class InternalApiController {
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> credentials) {
        ...
        log.info("Tentativa de login interno para usuário (Caixa): {}", username);

        Optional<CoreUser> userOpt = coreUserRepository.findByUsername(username);

        if (userOpt.isPresent() && userOpt.get().getPassword().equals(password)) {
            log.info("Login interno bem-sucedido para (Caixa): {}", username);
        ...
        log.warn("Falha no login interno para (Caixa): {}", username);
        return ResponseEntity.ok(Map.of("valid", false));
    }

    @GetMapping("/accounts")
    public ResponseEntity<List<Map<String, Object>>> listAccounts(@RequestParam("user") Long userId) {
        log.info("Listando contas para o userId interno (Caixa): {}", userId);
    ...
    public ResponseEntity<?> getBalance(@PathVariable String accountId) {
        log.info("Consultando saldo para conta interna (Caixa): {}", accountId);
    ...
    public ResponseEntity<?> getTransactions(@PathVariable String accountId) {
        log.info("Consultando histórico de transações para conta (Caixa): {}", accountId);
        ...
        if (accountOpt.isPresent()) {
            List<com.caixa.api.core.model.CoreTransaction> transactions = coreTransactionRepository.findByAccountOrderByTimestampDesc(accountOpt.get());
