...
public class PartyService {
    ...
    public void registerParticipant(String idType, String idValue) {
        ...
        try {
            log.info("A registar participante no ALS via SDK (BCA): {}/{} currency={}", idType, idValue, currency);
            restTemplate.postForEntity(sdkOutboundUrl + "/accounts", java.util.List.of(entry), Map.class);
        } catch (Exception e) {
            log.error("Erro ao registar participante no ALS (BCA): {}", e.getMessage());
        }
    }
