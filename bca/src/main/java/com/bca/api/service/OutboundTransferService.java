...
public class OutboundTransferService {
    ...
    public TransferInitiateResponse initiateTransfer(TransferInitiateRequest request) {
        ...
        try {
            log.info("Iniciando Step 1 (Lookup) no SDK [BCA]: from={} to={}", fromMsisdn, request.getToAccount());
            ...
            log.error("Erro no Step 1 [BCA]: {}", e.getMessage());
    ...
    public TransferConfirmPartyResponse confirmParty(String transferId) {
        ...
        try {
            log.info("Iniciando Step 2 (Accept Party) no SDK [BCA] para ID: {}", transferId);
            ...
            log.error("Erro no Step 2 [BCA]: {}", e.getMessage());
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage());
        }
    }
    ...
    public TransferConfirmQuoteResponse confirmQuote(String transferId) {
        ...
        try {
            log.info("Iniciando Step 3 (Accept Quote) no SDK [BCA] para ID: {}", transferId);
            ResponseEntity<Map> response = restTemplate.exchange(
                    sdkOutboundUrl + "/transfers/" + transferId,
                    HttpMethod.PUT,
                    new HttpEntity<>(body),
            ...
            if ("COMMITTED".equals(currentState) || "COMPLETED".equals(currentState)) {
                log.info("[BCA] Step 3 concluído com sucesso. Processando débito local...");
            ...
            log.error("Erro no Step 3 [BCA]: {}", e.getMessage());
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, e.getMessage());
        }
    }
}
