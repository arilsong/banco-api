...
public class QuoteService {
    ...
    public QuoteResponse processQuote(QuoteRequest request) {
        ...
        String currency = parsed.getCurrency() != null ? parsed.getCurrency() : "EUR";
        ...
        if (request.getTo() != null && request.getTo().getIdValue() != null) {
            ...
            CoreAccount payeeAccount = partyService.findAccountByIdType(payeeType, payeeIdValue)
                    .orElseThrow(() -> {
                        log.warn("Conta/Alias de destino não encontrado [BNI]: {} {}", payeeType, payeeIdValue);
                        return new ResponseStatusException(HttpStatus.NOT_FOUND, "Conta/Alias de destino não encontrado.");
                    });
            log.info("Cliente destino verificado [BNI]: {} (Account: {})", payeeAccount.getDisplayName(), payeeAccount.getAccountNumber());
        }

        return QuoteResponse.builder()
                .quoteId(request.getQuoteId())
                .transactionId(request.getTransactionId())
                .transferAmount(new java.math.BigDecimal(amountValue).stripTrailingZeros().toPlainString())
                .transferAmountCurrency(currency)
                .payeeReceiveAmount(new java.math.BigDecimal(amountValue).stripTrailingZeros().toPlainString())
